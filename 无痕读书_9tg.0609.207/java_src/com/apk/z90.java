package com.apk;

import android.view.View;
import com.tr.comment.sdk.ggs.view.TrAdViewBanner;
/* compiled from: TrAdViewBanner.java */
/* loaded from: classes7.dex */
public class z90 implements x60 {

    /* renamed from: do  reason: not valid java name */
    public final /* synthetic */ TrAdViewBanner f6153do;

    public z90(TrAdViewBanner trAdViewBanner) {
        this.f6153do = trAdViewBanner;
    }

    @Override // com.apk.b70
    /* renamed from: case */
    public void mo157case() {
        this.f6153do.f10422else = true;
    }

    @Override // com.apk.x60
    /* renamed from: do */
    public void mo164do() {
        this.f6153do.f10422else = true;
    }

    @Override // com.apk.b70
    /* renamed from: else */
    public void mo158else() {
    }

    @Override // com.apk.b70
    /* renamed from: if */
    public void mo159if(int i, String str) {
        this.f6153do.f10422else = true;
    }

    @Override // com.apk.x60
    /* renamed from: new */
    public void mo165new(View view) {
        TrAdViewBanner.m3709if(this.f6153do, view);
    }

    @Override // com.apk.x60
    public void onAdClick() {
    }
}
