package com.apk;
/* compiled from: AdChapterUtils.java */
/* renamed from: com.apk.public  reason: invalid class name */
/* loaded from: classes8.dex */
public class Cpublic implements w60 {

    /* renamed from: do  reason: not valid java name */
    public final /* synthetic */ Cimport f3750do;

    public Cpublic(Cimport cimport) {
        this.f3750do = cimport;
    }

    @Override // com.apk.b70
    /* renamed from: case */
    public void mo157case() {
    }

    @Override // com.apk.w60
    /* renamed from: do */
    public void mo448do() {
    }

    @Override // com.apk.b70
    /* renamed from: else */
    public void mo158else() {
    }

    @Override // com.apk.w60
    /* renamed from: for */
    public void mo449for() {
        this.f3750do.f2176finally = true;
    }

    @Override // com.apk.b70
    /* renamed from: if */
    public void mo159if(int i, String str) {
        d60 d60Var = this.f3750do.f2174extends;
        if (d60Var != null) {
            d60Var.m409case();
            this.f3750do.f2174extends = null;
        }
    }

    @Override // com.apk.w60
    public void onAdClick() {
    }

    @Override // com.apk.w60
    public void onAdClosed() {
        d60 d60Var = this.f3750do.f2174extends;
        if (d60Var != null) {
            d60Var.m409case();
            this.f3750do.f2174extends = null;
        }
    }
}
