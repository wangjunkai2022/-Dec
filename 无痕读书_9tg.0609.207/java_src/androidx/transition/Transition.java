...
public abstract class Transition implements Cloneable {
    ...
    public static int[] parseMatchOrder(String str) {
        ...
        while (stringTokenizer.hasMoreTokens()) {
            ...
            if ("id".equalsIgnoreCase(trim)) {
                ...
                throw new InflateException(Cgoto.m996else("Unknown match type in matchOrder: '", trim, "'"));
            }
            i++;
        }
        return iArr;
    ...
    public String toString(String str) {
        ...
        m1016super.append(getClass().getSimpleName());
        m1016super.append("@");
        m1016super.append(Integer.toHexString(hashCode()));
        m1016super.append(": ");
        String sb = m1016super.toString();
        if (this.mDuration != -1) {
            StringBuilder m1025while = Cgoto.m1025while(sb, "dur(");
            m1025while.append(this.mDuration);
            m1025while.append(") ");
            sb = m1025while.toString();
        }
        if (this.mStartDelay != -1) {
            StringBuilder m1025while2 = Cgoto.m1025while(sb, "dly(");
            m1025while2.append(this.mStartDelay);
            m1025while2.append(") ");
            sb = m1025while2.toString();
        }
        if (this.mInterpolator != null) {
            StringBuilder m1025while3 = Cgoto.m1025while(sb, "interp(");
            m1025while3.append(this.mInterpolator);
            m1025while3.append(") ");
            sb = m1025while3.toString();
        }
        if (this.mTargetIds.size() > 0 || this.mTargets.size() > 0) {
            String m989case = Cgoto.m989case(sb, "tgts(");
            if (this.mTargetIds.size() > 0) {
                for (int i = 0; i < this.mTargetIds.size(); i++) {
                    if (i > 0) {
                        m989case = Cgoto.m989case(m989case, ", ");
                    }
                    StringBuilder m1016super2 = Cgoto.m1016super(m989case);
                    m1016super2.append(this.mTargetIds.get(i));
                    m989case = m1016super2.toString();
                }
            }
            if (this.mTargets.size() > 0) {
                for (int i2 = 0; i2 < this.mTargets.size(); i2++) {
                    ...
                    StringBuilder m1016super3 = Cgoto.m1016super(m989case);
                    m1016super3.append(this.mTargets.get(i2));
                    m989case = m1016super3.toString();
                }
            }
            return Cgoto.m989case(m989case, ")");
        }
        return sb;
    }
