...
public class SuggestionsAdapter extends ResourceCursorAdapter implements View.OnClickListener {
    ...
    private Drawable getDrawableFromResourceValue(String str) {
        if (str == null || str.isEmpty() || SpeechSynthesizer.REQUEST_DNS_OFF.equals(str)) {
            return null;
        }
        try {
                ...
                if (pathSegments != null) {
                    ...
                    if (size == 1) {
                        try {
                            ...
                            throw new FileNotFoundException(Cgoto.m1008new("Single path segment is not a resource ID: ", uri));
                        }
                    } else if (size == 2) {
                        parseInt = resourcesForApplication.getIdentifier(pathSegments.get(1), pathSegments.get(0), authority);
                    } else {
                        throw new FileNotFoundException(Cgoto.m1008new("More than two path segments: ", uri));
                    }
                    if (parseInt != 0) {
                        return resourcesForApplication.getDrawable(parseInt);
                    }
                    throw new FileNotFoundException(Cgoto.m1008new("No resource found for: ", uri));
                }
                throw new FileNotFoundException(Cgoto.m1008new("No path: ", uri));
            } catch (PackageManager.NameNotFoundException unused2) {
                throw new FileNotFoundException(Cgoto.m1008new("No package found for authority: ", uri));
            }
        }
        throw new FileNotFoundException(Cgoto.m1008new("No authority: ", uri));
    }
