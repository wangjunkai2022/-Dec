...
public class GlobalHistogramBinarizer extends Binarizer {
    ...
    public BitMatrix getBlackMatrix() throws NotFoundException {
        ...
        for (int i = 1; i < 5; i++) {
            ...
            for (int i3 = width / 5; i3 < i2; i3++) {
                int i4 = (row[i3] & ExifInterface.MARKER) >> 3;
                iArr[i4] = iArr[i4] + 1;
            }
        }
        ...
        for (int i5 = 0; i5 < height; i5++) {
            ...
            for (int i7 = 0; i7 < width; i7++) {
                if ((matrix[i6 + i7] & ExifInterface.MARKER) < estimateBlackPoint) {
                    bitMatrix.set(i7, i5);
                }
            }
        }
    ...
    public BitArray getBlackRow(int i, BitArray bitArray) throws NotFoundException {
        ...
        for (int i2 = 0; i2 < width; i2++) {
            int i3 = (row[i2] & ExifInterface.MARKER) >> 3;
            iArr[i3] = iArr[i3] + 1;
        }
        int estimateBlackPoint = estimateBlackPoint(iArr);
        if (width < 3) {
            for (int i4 = 0; i4 < width; i4++) {
                if ((row[i4] & ExifInterface.MARKER) < estimateBlackPoint) {
                    bitArray.set(i4);
                }
            }
        } else {
            int i5 = row[0] & ExifInterface.MARKER;
            int i6 = 1;
            int i7 = i5;
            int i8 = row[1] & ExifInterface.MARKER;
            while (i6 < width - 1) {
                int i9 = i6 + 1;
                int i10 = row[i9] & ExifInterface.MARKER;
